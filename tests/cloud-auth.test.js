...
test('signup delegates to auth without injecting privileges or duplicating backend records', async () => {
  ...
  assert.deepEqual(sent, {email:'a@example.test',password:'password',options:{emailRedirectTo:authReturnUrl(page),data:{terms_accepted:true,terms_version:'0.3-beta',privacy_version:'0.4-beta',analytics_consent:false,ads_consent:false,personalized_ads_consent:false}}});
});
