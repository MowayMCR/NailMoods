...
const make=(storage,repo,userId='A',workspaceId='WA',other={})=>createAccountStore({storage,repo,userId,workspaceId,...other});
...
test('offline draft and queue survive reload; retry uses same record id without duplicates',async()=>{
 ...
 store=make(storage,repo);assert.equal(JSON.parse(store.storage.getItem(COLLECTION))[0].name,'Prune');repo.fail=false;await store.load();assert.equal(await store.flush(),true);assert.equal(repo.rows.user_products.length,1);await store.flush();assert.equal(repo.rows.user_products.length,1);
});
...
test('storage quota fails before acknowledging or sending an unsaved draft',async()=>{
 const local=memory(),repo=backend(),store=make(local,repo);await store.load();local.setItem=()=>{throw new Error('quota');};assert.throws(()=>store.storage.setItem(COLLECTION,JSON.stringify([polish])),/quota/);assert.equal(repo.writes.length,0);assert.deepEqual(JSON.parse(store.storage.getItem(COLLECTION)),[]);
});
test('another tab modifying the cache prevents stale overwrite',async()=>{
 const local=memory(),repo=backend(),store=make(local,repo);await store.load();local.setItem(accountCacheKey('A','WA'),'{}');assert.throws(()=>store.storage.setItem(COLLECTION,JSON.stringify([polish])),/autre onglet/);
});
...
test('explicit remote reload preserves a backup of conflicting drafts before clearing the queue',async()=>{
 const local=memory(),repo=backend(),store=make(local,repo);await store.load();repo.fail=true;store.storage.setItem(COLLECTION,JSON.stringify([polish]));await store.flush();assert.equal(store.pending,1);
 repo.fail=false;const writes=[];const original=local.setItem;local.setItem=(k,v)=>{writes.push(k);original(k,v);};await store.load({useRemote:true});assert.equal(store.pending,0);assert.deepEqual(JSON.parse(store.storage.getItem(COLLECTION)),[]);assert.ok(writes.some(k=>k.includes(':backup:')));
});
