-- Phase 12 production compatibility: existing accounts predate the explicit
-- 18+ declaration. Never infer adulthood from an existing account or from
-- accepting optional privacy choices; require an explicit declaration.
begin;

drop function if exists public.record_privacy_choices(text,text,boolean,boolean,boolean,boolean);
drop function if exists private.record_privacy_choices(text,text,boolean,boolean,boolean,boolean);

create function private.record_privacy_choices(
  ...
  p_personalized_ads_consent boolean
)
...
begin
  ...
  if p_privacy_version is distinct from '0.1-beta'
  ...
  if coalesce(p_analytics_consent,false)
     or coalesce(p_ads_consent,false)
...
revoke all on function private.record_privacy_choices(text,text,boolean,boolean,boolean,boolean,boolean)
  from public, anon;
grant execute on function private.record_privacy_choices(text,text,boolean,boolean,boolean,boolean,boolean)
  to authenticated;

create function public.record_privacy_choices(
  p_privacy_version text,
  p_terms_version text,
  p_accept_terms boolean,
  p_confirm_adult boolean,
...
set search_path=''
as $$
  select private.record_privacy_choices(
    p_privacy_version,
    p_terms_version,
    p_accept_terms,
    p_confirm_adult,
    p_analytics_consent,
    p_ads_consent,
    p_personalized_ads_consent
  )
$$;

revoke all on function public.record_privacy_choices(text,text,boolean,boolean,boolean,boolean,boolean)
  from public, anon;
grant execute on function public.record_privacy_choices(text,text,boolean,boolean,boolean,boolean,boolean)
  to authenticated;
