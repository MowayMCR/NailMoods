...
begin
 if not private.discovery_allowed() then raise exception 'plus_or_pro_required' using errcode='42501';end if;
  ...
  where p.username=normalized_handle and private.effective_tier(p.id) in ('plus','pro') and (
...
do $$declare t text;begin foreach t in array array['user_products','user_stickers','user_equipment'] loop
 execute format('create policy p2_personal_read on public.%I as restrictive for select to authenticated using(created_by=(select auth.uid()) or (select private.tier_allows(true)))',t);
end loop;end $$;
