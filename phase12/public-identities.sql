...
language sql stable security definer set search_path='' as $$
 ...
 select 'user'::text as entity_type,p.id as entity_id,p.username as handle,p.display_name,'plus'::text as kind,
 null::text as avatar_url,null::text as city,null::text as bio,array[]::text[] as styles
 from public.profiles p cross join request r where p.account_tier='plus' and p.username is not null
