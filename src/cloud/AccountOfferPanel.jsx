...
export default function AccountOfferPanel({client,userId,store,onApplied}){
 ...
 async function choose(){
  ...
  try{
   ...
   const next=await service.choose(selected);setState(next);setAccept(false);setAdult(false);
   await onApplied();
 ...
 return <section id="account-offer" className="card accountOffer" aria-labelledby="account-offer-title">
  ...
  <small className="offerDataNote">Changer d’offre ne supprime jamais ta collection, ton journal ni ton espace Pro. Une offre expirée pourra être verrouillée plus tard sans effacer les données.</small>
