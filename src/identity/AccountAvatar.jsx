...
import {dataUrlToBlob} from '../cloud/mediaStorage';
...
export default function AccountAvatar({client,userId,workspaceId}){
 ...
 async function change(file){setBusy(true);setNotice('');try{
  ...
  if(file){file=dataUrlToBlob(await preparePhoto(file,1000));validateImage(file);const uploaded=await media.upload({userId,workspaceId,kind:'avatar',objectId:crypto.randomUUID(),file});await media.signedUrl(uploaded.path);next=uploaded.path;}
 ...
 }catch(error){setNotice(error.message || 'La photo n’a pas pu être enregistrée. Réessaie.');}finally{setBusy(false);}}
