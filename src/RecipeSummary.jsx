...
export default function RecipeSummary({ idea, standalone = false }) {
  if (!idea) return null;
  ...
  return <details className="recipeSummary"><summary>Ma recette de pose</summary>
    ...
    <h3>Étapes essentielles</h3><ol>{steps.map(step=><li key={step.id}><b>{step.title}</b><p>{step.body}</p>{step.hint && <small>{step.hint}</small>}</li>)}</ol><p>Répète la pose sur l’autre main.{!standalone && ' Le tutoriel conserve ta progression étape par étape.'}</p>
