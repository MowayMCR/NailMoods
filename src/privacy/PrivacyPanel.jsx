...
export default function PrivacyPanel({ client, userId, tier, guestStorage, onDeleted, localDraft, embedded = false, onBack }) {
  ...
  return <section id="privacy-settings" className={`privacySection${embedded ? ' privacyEmbedded' : ' card'}`}>{onBack && <button className="privacyBack" type="button" onClick={onBack}>← Retour à mon compte</button>}<h2>Confidentialité</h2><p>Tes choix restent les tiens. Refuser les options ne bloque pas NailMoods.</p>
    {needsTerms && <section className="privacyConfirmation" role="status"><p className="privacyConfirmationTitle">Mise à jour à confirmer</p><p>Une nouvelle version des textes est disponible. Tes usages personnels restent accessibles ; confirme-la lorsque tu souhaites modifier ton offre ou tes réglages sociaux.</p><LegalLinks/><label>Tranche d’âge<select value={ageBand || record?.age_band || ''} onChange={e => setAgeBand(e.target.value)}><option value="">Choisir…</option><option value="15_17">J’ai entre 15 et 17 ans · compte Free personnel</option><option value="18_plus">J’ai 18 ans ou plus</option></select></label><label className="consentCheck"><input type="checkbox" checked={accept} onChange={e => setAccept(e.target.checked)}/>J’accepte les Conditions d’utilisation · version {TERMS_VERSION}</label><button className="privacyConfirmButton" disabled={!accept || !(ageBand || record?.age_band) || busy} onClick={() => save(choices, true, ageBand || record?.age_band)}>Confirmer et enregistrer</button></section>}
