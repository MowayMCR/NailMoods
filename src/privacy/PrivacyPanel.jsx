...
export default function PrivacyPanel({ client, userId, tier, guestStorage, onDeleted, localDraft }) {
  ...
  const [busy, setBusy] = useState(false), [loaded, setLoaded] = useState(!userId), [notice, setNotice] = useState(''), [confirmation, setConfirmation] = useState(''), [accept, setAccept] = useState(false);
  useEffect(() => { let cancelled = false;
    setChoices(DENIED); setRecord(null); setLoaded(!userId); setNotice(''); setConfirmation('');
  ...
  async function save(next = choices, acceptTerms = false) {
    setBusy(true); setNotice('');
    try {
      const safe = availableChoices(next);
      const value = userId ? await privacyService(client).save(safe, acceptTerms) : { ...safe, privacy_version: PRIVACY_VERSION, consent_updated_at: new Date().toISOString() };
  ...
  return <section className="card privacySection"><h2>Confidentialité</h2><p>Tes choix restent les tiens. Refuser les options ne bloque pas NailMoods.</p>
    {needsTerms && <div role="status"><p>Une nouvelle version des conditions est disponible. Tu peux continuer à explorer sans accepter.</p><LegalLinks/><label className="consentCheck"><input type="checkbox" checked={accept} onChange={e => setAccept(e.target.checked)}/>J’accepte les Conditions d’utilisation, version {TERMS_VERSION}</label><button disabled={!accept || busy} onClick={() => save(choices, true)}>Enregistrer mon acceptation</button></div>}
