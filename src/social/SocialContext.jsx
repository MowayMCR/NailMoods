...
export async function socialCall(client,action,data={}) {
 const r=await client.rpc('nm_social',{p_action:action,p_data:data});if(r.error){if(r.error.message?.includes('FEATURE_REQUIRES'))track('feature_locked',{});throw r.error;}const events={request:'connection_request_sent',accept:'connection_request_accepted',decline:'connection_request_declined',remove:'connection_removed',send:'message_sent',open:'conversation_opened'};if(events[action])track(events[action],{});return r.data;
}
