...
export default function PublicationTags({source,value,onChange}){
 ...
 return <section className="publicationTags"><h3>L’ambiance de cette publication</h3><p>Suggestions à confirmer d’après ta pose et ses couleurs. La photo peut aussi contenir des couleurs de fond.</p><div className="tagChips">{tags.moods.map(v=><button type="button" aria-label={'Retirer '+v} key={v} aria-pressed="true" onClick={()=>toggle('moods',v)}>{v} ×</button>)}{!tags.moods.length&&<small>Choisis une ambiance pour aider à retrouver ta publication.</small>}</div><details><summary>Choisir ou modifier les ambiances</summary><div className="tagChips">{TAXONOMY.moods.map(v=><button type="button" key={v} aria-pressed={tags.moods.includes(v)} onClick={()=>toggle('moods',v)}>{v}</button>)}</div></details><details><summary>Autres tags · facultatifs</summary>{Object.entries(TAXONOMY).filter(([key])=>key!=='moods').map(([key,values])=><fieldset key={key}><legend>{TAG_LABELS[key]}</legend><div className="tagChips">{values.map(v=><button type="button" key={v} aria-pressed={tags[key].includes(v)} onClick={()=>toggle(key,v)}>{v}</button>)}</div></fieldset>)}<small>Confirme une technique seulement si tu la connais.</small></details><button type="button" className="publicationSuggestionsAction" onClick={()=>{edited.current=true;onChange(suggestions);setNotice('Suggestions appliquées : ajuste-les avant de publier.');}}><RotateCcw aria-hidden="true"/><span>Reprendre les suggestions</span></button>{notice&&<small role="status">{notice}</small>}</section>;
}
