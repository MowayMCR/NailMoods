...
export default function ProfileView({ accountAccess, extras, onCreate, onEquipment, onFavorites, profile, items, onChange, onCollection, personalModel, personalSettings, onPersonalization }) {
  ...
  return <div className="profilePage">
    ...
    {universes && <Sheet title="Explorer les univers" onClose={() => setUniverses(false)} className="profileSheet universePicker"><div className="search"><Search /><input aria-label="Rechercher un univers" value={query} onChange={event => setQuery(event.target.value)} placeholder="Rechercher un univers…" /></div>{Object.entries(universeGroups).map(([group, values]) => <div className="universeGroup" key={group}><h3>{group}</h3><div className="chips">{values.filter(style => style.toLocaleLowerCase('fr').includes(query.toLocaleLowerCase('fr'))).map(style => <button key={style} className={profile.styles.includes(style) ? 'on' : ''} aria-pressed={profile.styles.includes(style)} onClick={() => toggleStyle(style)}><MoodGlyph value={style} />{style}</button>)}</div></div>)}</Sheet>}
  </div>;
}
